...
const SITE_NAME = "USADOC"
// SENDER_DOMAIN is the verified sender subdomain FQDN (e.g., "notify.example.com").
// It MUST match the subdomain delegated to Lovable's nameservers. NEVER use the root domain.
const SENDER_DOMAIN = "notify.usadoc.net"
// FROM_DOMAIN is the domain shown in the From: header (e.g., "example.com").
// Can be the root domain when display_from_root is enabled — this is cosmetic only.
const FROM_DOMAIN = "notify.usadoc.net"
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Verify the caller. Two valid auth modes:
        //   1. Bearer <user JWT>     — validated via supabase.auth.getUser()
        //   2. Bearer <service_role> — server-to-server (Edge Functions, cron, etc.)
